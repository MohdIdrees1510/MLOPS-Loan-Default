...
class MyModel:
    def __init__(self, trained_model_object: object, preprocessing_object: Pipeline = None):
        """
        :param trained_model_object: Input Object of trained model 
        :param preprocessing_object: Optional preprocessing pipeline (can be None)
        """
        self.trained_model_object = trained_model_object
        self.preprocessing_object = preprocessing_object
    ...
    def predict(self, dataframe: pd.DataFrame) -> DataFrame:
        """
        Function accepts inputs, applies optional preprocessing, and performs prediction.
        """
        try:
            logging.info("Starting prediction process.")

            # Step 1: Apply preprocessing if available
            if self.preprocessing_object:
                logging.info("Applying preprocessing transformations")
                dataframe = self.preprocessing_object.transform(dataframe)
            else:
                logging.info("No preprocessing object found — skipping transformation")
